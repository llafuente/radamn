    ...
    Node.Implements({
        ...
        */
        getBoundingBox: function (with_offset) {
            ...
            if (with_offset) {
                bb[0] = (bb[0] * scl[0]) - this.offset[0];
                bb[1] = (bb[1] * scl[1]) - this.offset[1];
                bb[2] = (bb[2] * scl[0]) - this.offset[0];
                bb[3] = (bb[3] * scl[1]) - this.offset[1];
            } else {
                bb[0] = (bb[0] * scl[0]);
                bb[1] = (bb[1] * scl[1]);
                bb[2] = (bb[2] * scl[0]);
        ...
        getWorldBoundingBox: function(with_offset) {
            var bb = this.getBoundingBox(with_offset);
        ...
        behavior: function (type, options) {
            ...
            case "draggable":
                ...
                Radamn.on("pointerdown", function (event) {
                    var collision;

                    if (options.click) {
                        collision = this.ray(event.x, event.y);
                        console.log("collision", collision);
                        collision = collision.reason === INSIDE;
                ...
                Radamn.on("pointerup", function (event) {
                    if (draggin === true) {
                        draggin = false;
                        start_event = null;
                        last_event = null;
                ...
                Radamn.on("pointermove", function (event) {
                    if (draggin === true && event.finger === start_event.finger) {
                        var x = options.x ? event.x - last_event.x : 0,
                            y = options.y ? event.y - last_event.y : 0
                        ;
                        this.translate(x, y);
                        this.emit("drag", [[x, y]]);
                ...
                break;
            case "align":
                this.on("tick", function(delta, updated) {
                    if (updated) {
                        this.positionAlign(options.align, options.bb);
                    }
                }.bind(this));

(function (exports, browser) {
    ...
    Node.Implements({
        ...
        translate: function(x, y) {
            var out = Matrix2D.translate(this.matrix, this.matrix, [x, y]);
        },
        ...
        applyTransform: function (ctx) {
            ...
            if (this.center_offset !== null) {
                ctx.transform(p[0], p[1], p[2], p[3], p[4] - this.center_offset[0], p[5] - this.center_offset[1]);
            } else {
        ...
        setCenter: function (c) {
            ...
            this.center = c = c || this.center;

            switch (c) {
            case Node.topLeft:
                // do nothing!
                this.center_offset = null;
                break;
            case Node.topMiddle:
                bb = this.getBoundingBox();
                this.center_offset = [(bb[2] * 0.5), 0];
                break;
            case Node.topRight:
                bb = this.getBoundingBox();
                this.center_offset = [bb[2], 0];
                break;

            case Node.centerLeft:
                bb = this.getBoundingBox();
                this.center_offset = [0, (bb[2] * 0.5)];
                break;
            case Node.center:
                bb = this.getBoundingBox();
                this.center_offset = [(bb[2] * 0.5), (bb[2] * 0.5)];
                this.center_offset = [(bb[2] * 0.5), (bb[2] * 0.5)];
                break;
            case Node.centerRight:
                bb = this.getBoundingBox();
                this.center_offset = [bb[2], (bb[2] * 0.5)];
                break;

            case Node.bottomLeft:
                bb = this.getBoundingBox();
                this.center_offset = [0, bb[2]];
                break;
            case Node.bottomMiddle:
                bb = this.getBoundingBox();
                this.center_offset = [(bb[2] * 0.5), bb[2]];
                break;
            case Node.bottomRight:
                bb = this.getBoundingBox();
                this.center_offset = [bb[2], bb[2]];
                break;
        ...
        */
        ...
        visibilityCheck: function() {
            if(this.__gbody.length !== 0) {
                this.__visibility = Radamn.getScene().isVisible(this.__gbody[0]);
            }
        }
    });
    ...
    function renderer(element, property, value, unit) {
        ...
        case 'skewx':
            aux = value.split(" ");
            element.skewx(aux[0], false);
            break;
        case 'skewy':
            aux = value.split(" ");
            element.skewy(false, aux[0]);
            break;
        }
    }

...
(function (exports, browser) {
    ...
    if(!browser) {
         ...
         * @ignore
         */
        global.document = document = (function() {
            var events = null;
            return {
    ...
    (function() {
        ...
        if(!DomWindow.requestAnimationFrame) {
            DomWindow.requestAnimationFrame = function(fn, delay) {
                DomWindow.interval = setTimeout(fn, delay);
            };
            DomWindow.cancelAnimationFrame = function() {
                clearTimeout(DomWindow.interval);
            };
        } else {
    ...
    /** @lends Radamn.prototype */
    {
        ...
        listenInput: function(delay) {
            var bind_event, emit_event;

            if(browser) {
                emit_event = function(event) {
                    this.emit(event.type, event);
                };
                emit_event = emit_event.bind(this);

                bind_event = function(v) {
                    window.document["on"+v] = emit_event;
                };
                bind_event = bind_event.bind(this);

                ["mousedown", "mouseup", "keydown", "keyup", "mousemove", "mouseenter", "mouseleave"].forEach(bind_event);
            }
    ...
    if(browser) {
        ...
        CRadamn.init();
        exports.Radamn = global.Radamn = new Radamn();

...
(function(exports, browser) {
    ...
    var TMXIsometric = /** @lends TMXIsometric.prototype */
    {
        ...
        console.debug("getOrientation", tiledest, tile);
            if(tiledest.y > tile.y) return "east";
            if(tiledest.y < tile.y) return "west";
            if(tiledest.x > tile.x) return "south";
            return "north";
        },
        /**
    ...
    var TMXOrtogonal = /** @lends TMXOrtogonal.prototype */
    {
         ...
         */
        getOrientation: function (tiledest, tile) {
            if(tiledest.y > tile.y) return "north";
            if(tiledest.y < tile.y) return "south";
            if(tiledest.x > tile.x) return "east";
            return "west";
        },
        //todo
        getOrientationOpposite: function(orientation) {
    ...
    TMX.implements({
        ...
        __optimizeLayers: function () {
            console.info("[TMX] optimizing layers");

            // per layer!
            var i = 0,
                max = this.layers.length;
            for (; i < max; ++i) {
                console.info("[TMX] post process layer: " + i);
        ...
        draw: function (ctx, elapsed_time) {
            ...
            for (; i < max; ++i) {
                //this.layers[i].tiles = null;
                //this.layers[i].raw = null;
                //console.log(JSON.encode(this.layers[i]));

                if(this.layers[i].show === false) continue;

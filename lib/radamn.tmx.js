...
(function (exports) {
    ...
    var TMXIsometric = /** @lends TMXIsometric.prototype */
    {
        ...
        getTilePosition: function (tileset, x, y, as_float) {
            ...
            tempX = (x / tileset.tilewidth); // = (mapPoint.X + mapPoint.Y);
            tempY = (y / tileset.tileheight * 0.5); // = (mapPoint.X -
            // mapPoint.Y);
            if (!as_float) {
                return {
    ...
    var TMX = new Class( /** @lends TMX.prototype */ {
                ...
                parser.onopentag = function (tag) {
                    ...
                    // do not clone it!
                      this.tilesets[this.tilesets.length - 1].image = Radamn.Assets.getImage(tag.attributes.source);
                        ...
                        break;
                    case 'layer':
                        this.__pushLayer("tiles", tag.attributes.name);
                        layer = true;
                        break;
                    case 'objectgroup':
                        this.__pushLayer("objects", tag.attributes.name);
                        objects = true;
                        break;
         ...
         * @params {String} type
         */
        __pushLayer: function (type, name) {
            this.layers.push({
                type: type,
                name: name,
            ...
            for (; i < max; ++i) {
                ...
                case 'tiles':
                    ...
                    for (; j < jmax; ++j) {
                        ...
                        var tile_data = {
                            image: tileset.image,
                            x: tile.tile.id % tileset.x,
                            y: Math.floor(tile.tile.id / tileset.x),
                            position: this.getScreenPosition(tileset, tile.position.x, tile.position.y, 0),
                        };
                        ...
                        tileset.image,
                        tile_data.x * tileset.tilewidth,
                        tile_data.y * tileset.tileheight,
                        tileset.tilewidth,
                        tileset.tileheight,
        ...
        ray: function(mouse_ev) {
            // get Global position!
            if(this.parentNode !== null) {
                var translation = this.parentNode.getDerivedPosition();
                mouse_ev.x -= translation.x;
                mouse_ev.y -= translation.y;
            }

            var tileset = this.tilesets[0];

            //force the tileset...
            var tilepos = this.getTilePosition(tileset, mouse_ev.x, mouse_ev.y);
            console.log(tilepos);
            ...
            //cache
            var tileset_w = tileset.tilewidth,
            tileset_h = tileset.tileheight,
            ok_tiles = [];
            ...
            for(;i<max;++i) {
                ...
                if(tiles !== null) {
                    ...
                    for(;j<jmax;++j) {
                        ...
                        var offset = {
                                x: ( mouse_ev.x - tile_top_left.x ),
                                y: ( mouse_ev.y - tile_top_left.y )
                        };
...
})(typeof exports === "undefined" ? radamn.tmx = {} : exports);